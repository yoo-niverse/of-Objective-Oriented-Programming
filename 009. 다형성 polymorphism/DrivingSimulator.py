...
class DrivingSimulator:
    ...
    def add_vehicle(self, new_vehicle):
        ...
        if isinstance(new_vehicle, Vehicle):
            self.mobility.append(new_vehicle)
        else:
    ...
    def __str__(self):
        ...
        return ''
            ''' for문을 통해 여러 문자열을 출력한 후, return 처리가 없으면 오류가 발생한다.
            따라서 White Space를 반환해주어 오류를 방지하고, 모든 문자열이 출력되기 이전에 메소드 실행이 종료되는 것을 방지한다. '''
