...
class Exporter(ABC):
    ''' ExportController 클래스의 set_exporter 메소드가 CSVExporter 클래스에 의존하지 않도록
     추상 클래스를 만들어 export 메소드를 필수적으로 오버라이딩 하도록 설정하였다.

     이를 상속받는 CSV, HTMLExporter에서는 필수적으로 동명의 메소드를 오버라이딩해야하며,
     새로운 변환기가 추가 되더라도 컨트롤러 클래스를 수정할 필요없이 클래스만 추가하면 되도록 구현할 수 있었다. '''
    @abstractmethod
