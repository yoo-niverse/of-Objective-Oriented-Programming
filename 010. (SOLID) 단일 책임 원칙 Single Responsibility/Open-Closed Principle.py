''' 개방 폐쇄 원칙의 적용을 통해, 코드 수정없이 기능 확장이 가능한 코드 구현 '''
...
class Message(ABC):
    ''' 카카오톡, 페이스북, 문자 메세지 클래스에 들어가는 필수 기능을 토대로
    추상 클래스를 활영하여 다형성을 구현하였다. '''
    ...
    def short_message(self):
        ''' 메세지를 단문으로 출력하는 short_message라는 이름으로의 오버라이딩을 강제하여,
        메소드명의 불일치로 오류가 발생하는 부분을 해결하였다.'''
        pass
